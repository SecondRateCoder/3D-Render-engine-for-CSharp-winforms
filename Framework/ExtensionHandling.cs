...
static class ExtensionHandler{
    ...
    static Dictionary<string, object> SettingsDeserialised;
    static readonly string[] SupportedFunctions = {"SetUp", "Update", "TimedUpdate", "Closing"};
    ...
    /// <summary>
    /// Initialise the public proprties of a extension's functions.
    /// </summary>
    /// <param name="Path">The path to the Extension's Metadata.json.</param>
    static void _Init(string Path){
#pragma warning disable CS8601
        JsonDeserialised = JsonSerializer.Deserialize<Dictionary<string, object>>(File.OpenRead(Path), JsonSerializerOptions.Default);
        SettingsDeserialised = JsonSerializer.Deserialize<Dictionary<string, object>>(File.OpenRead(StorageManager.ApplicationPath + @"Cache\Extensions.txt"), JsonSerializerOptions.Default);
#pragma warning restore CS8601
        //Decrement Path to the Parent directory and Slice it.
        int cc = Path.Length;
        while(Path[cc] != '\\' && cc >= 0){cc++;}
        extPath = new Span<char>(Path.ToCharArray()).Slice(0, cc).ToString();
    }
    public static void PreLoad_(){
        string Extensions = File.OpenText(StorageManager.ApplicationPath + @"Cache\Extensions.txt");
    }
}
...
// 	static async Task<ErrorQueue.SuccessState> LoadToMemory(string ClassName, string FunctionBody, string FunctionName, (MethodType mT, int Priority)[] Functions, int Recalls =0){
