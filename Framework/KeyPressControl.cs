...
class Joystickhandle{
	...
	void RecordButtonStart() {
		...
		for(int cc =0; cc < buttonsPressed.Length;cc++){
			if(buttonsPressed[cc]){ ButtonStart[cc] = DateTime.Now; }
		}
	}
	/// <summary>
	...
	int GetButtonPressLength(int index){ return DateTime.Now.Millisecond - ButtonStart[index].Millisecond; }
	...
	public void HandleKeyPress(InputController.KeyPressedDelegate delegateToHandle, Keys supposedKey){
		...
		for(int cc =0; cc < this.GetButtonNumber();cc++){
			...
			double normalisednetForce = Math.Sqrt((p.X - 32767)^2 + (p.Y - 32767)^2)/(32767 * Math.Sqrt(2));
